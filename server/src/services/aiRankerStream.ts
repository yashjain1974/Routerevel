    ...
    for (let i = 0; i < quality.length && totalEmitted < MAX_STOPS; i += BATCH_SIZE) {
        ...
        if (process.env.GROQ_API_KEY) {
            try {
                ...
                console.warn(`[STREAM] Groq batch failed, trying Gemini:`, err instanceof Error ? err.message : err)
            }
        }

        // Gemini fallback (20 req/day — save for when Groq fails)
        if (results.length === 0 && process.env.GEMINI_API_KEY) {
            try {
                results = await scoreBatchGemini(batch, context, globalRank)
            } catch (err) {
                console.warn(`[STREAM] Gemini batch failed:`, err instanceof Error ? err.message : err)
            }
        }
